    ...
    <header>
        <nav class="navbar navbar-default bg-success">
            <div class="container-fluid">
                <div class="navbar-header"><a class="navbar-brand" href="family_view.php?friend_id=<?=$friend_id?>"><font size=3>▶︎Back to Family</a></div>
...
<!-- $rowをvalueに追加 -->
<form method="POST" action="updatecd.php">
        <div class="jumbotron">
            <fieldset>
                <legend>Update family?</legend>
                <label>Child1：<input type="text" name="child1_name" value="<?=$row["child1_name"]?>"></label><br>
                <label>Child1 BirthDay：<input type="text" name="child1_bd" value="<?=$row["child1_bd"]?>"></label><br>
                <label>Child2：<input type="text" name="child2_name" value="<?=$row["child2_name"]?>"></label><br>
                <label>Child2 BirthDay：<input type="text" name="child2_bd" value="<?=$row["child2_bd"]?>"></label><br>
                <!-- idをこっそり渡す（hidden） -->
